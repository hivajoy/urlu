...
File Size: {}""".format(url, humanbytes(total_length))
        ...
        with open(file_name, "wb") as f_handle:
            while True:
                ...
                downloaded += len(chunk)  # Fix: increase by the chunk's actual size
                
                now = time.time()
                diff = now - start
                if diff >= 2:  # Update every 2 seconds instead of modulo logic
...
ETA: {}""".format(
    ...
    TimeFormatter(estimated_total_time)
                    ...
                    start = now  # Reset the start time for the next update
